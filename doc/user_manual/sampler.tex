...
\newcommand{\constantVariablesDescription}
{
  ...
  optional parameter} the user is able to input variables (parameters) that need to be 
  kept constant.    
  For doing this, as many \xmlNode{constant} nodes as needed can be 
  input. The  \xmlNode{constant} will specify one attribute:
  \begin{itemize}
    \item \xmlAttr{name}, \xmlDesc{required string attribute}, user-defined name
      of this constant.
  \end{itemize}
  For example, a constant variable can be inputted as follows:
  \begin{lstlisting}[style=XML]
   <variable name="aVariableName">
     ...
   </variable>
   <constant name="aConstant">1.23456789</constant>
\end{lstlisting}
...
\item \xmlNode{samplerInit},  \textit{\textbf{XML node, required parameter}}. In this xml-node,the following xml sub-nodes need to be specified:
  \begin{itemize}
    ...
    \item \xmlNode{samplingType}, \textit{\textbf{integer, optional field}}, sub-type of sampling
  \end{itemize}
\end{itemize}
\begin{itemize}
...
Regarding the \xmlNode{samplingType}, the user can choose to perform a Monte-Carlo sampling where the location of the samples in 
...
bound specified (i.e., \xmlNode{lowerBound} and \xmlNode{upperBound}).
